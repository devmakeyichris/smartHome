...
type Device = {id?: number; pin: number; status: boolean; type: 'light' | 'door'};
type Room = {id?: number; devices: Device[]; roomName: string};
type LogEntry = {action: string; id: string; room: string; time: string};
type DashboardTab = 'home' | 'activity' | 'access' | 'settings';
type ActivityFilter = 'all' | 'light' | 'door';
type SceneMode = 'away' | 'night' | 'welcome';
type AccessBadge = {id: string; label: string; owner: string; status: 'Autorise' | 'Suspendu' | 'A associer'};

type DashboardExperienceProps = {
  activeDevices: number;
  addDevice: (roomIndex: number, type: 'light' | 'door', pin: number) => Promise<boolean>;
...
const DashboardExperience = ({
  ...
  const [newDeviceType, setNewDeviceType] = useState<'light' | 'door'>('light');
