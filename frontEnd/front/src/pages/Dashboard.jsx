...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (savedConfig && JSON.parse(savedConfig).length > 0) {
        ...
        if (userData.rooms && userData.rooms.length > 0) {
          defaultConfig = userData.rooms.map((room, index) => ({
            roomName: room.name,
            lightPin: room.lightPin,
            doorPin: room.doorPin || null,
            hasMainDoor: room.isMainDoor || false,
            devices: [
              { type: 'LIGHT', status: false, pin: room.lightPin }
            ]
          }));
          
          defaultConfig = defaultConfig.map(room => {
            if (room.doorPin) {
              room.devices.push({ type: 'DOOR', status: false, pin: room.doorPin });
            }
            return room;
          });
          
          const mainDoorRoomObj = defaultConfig.find(room => room.hasMainDoor === true);
          if (mainDoorRoomObj) {
            setInitialMainDoorName(mainDoorRoomObj.roomName);
          }
        } else {
          defaultConfig = [
            {
              ...
              hasMainDoor: true,
              devices: [
                { type: 'LIGHT', status: false, pin: 4 },
                { type: 'DOOR', status: false, pin: 6 }
              ]
            },
            {
              roomName: "chambre",
              ...
              hasMainDoor: false,
              devices: [
                { type: 'LIGHT', status: false, pin: 8 }
              ]
            }
          ];
          setInitialMainDoorName("salon");
        }
        
        setRooms(defaultConfig);
        sessionStorage.setItem('homeConfig', JSON.stringify(defaultConfig));
        
        const mainDoor = defaultConfig.find(room => room.hasMainDoor === true);
        if (mainDoor) {
          setMainDoorRoom(mainDoor);
        }
      }
  ...
  const handleAddDevice = (e) => {
    ...
    room.devices.push({
      type: newDeviceType,
      status: false,
      pin: newDevicePin.trim()
    });
    
    saveConfigToStorage(updatedRooms);
    addLog('Ajouté', `Capteur ${newDeviceType} (PIN ${newDevicePin})`, room.roomName);
    toast.success(`Équipement ajouté dans ${room.roomName}`);
    setAddingDeviceRoomIdx(null);
    setNewDevicePin('');
  };
  
  const handleDeleteDevice = (roomIdx, deviceType) => {
    const updatedRooms = [...rooms];
    const room = updatedRooms[roomIdx];
    room.devices = room.devices.filter(d => d.type !== deviceType);
  ...
  const toggleDevice = async (roomIndex, deviceType) => {
    ...
    try {
      ...
      device.state = nextStatus
      ? (device.type === "LIGHT" ? "ON" : "OPEN")
      : (device.type === "LIGHT" ? "OFF" : "CLOSED");
